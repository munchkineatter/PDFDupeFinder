...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QPushButton, QFileDialog, QVBoxLayout, QWidget, QProgressBar, 
                             ...
                             QTextEdit, QWizard, QWizardPage, QCheckBox)
...
class DeleteConfirmationWizard(QWizard):
    def __init__(self, original, duplicates, parent=None):
        super().__init__(parent)
        self.original = original
        self.duplicates = duplicates
        self.setWindowTitle("Delete Files Confirmation")
        self.addPage(self.createConfirmationPage())
        self.setOption(QWizard.NoCancelButton, False)
        self.setWizardStyle(QWizard.ModernStyle)
        self.setFixedWidth(500)  # Adjust width as needed
    ...
    def createConfirmationPage(self):
        ...
        label = QLabel("Select the files you want to delete (at least one must remain):")
        layout.addWidget(label)

        self.checkboxes = []
        
        # Add original file
        original_checkbox = QCheckBox(self.original)
        original_checkbox.setChecked(False)
        layout.addWidget(original_checkbox)
        self.checkboxes.append(original_checkbox)
        ...
        # Connect checkboxes to validation function
        for checkbox in self.checkboxes:
            checkbox.stateChanged.connect(self.validateSelection)

        page.setLayout(layout)
        return page

    def validateSelection(self):
        # Ensure at least one file remains unchecked
        checked_count = sum(checkbox.isChecked() for checkbox in self.checkboxes)
        if checked_count == len(self.checkboxes):
            # If all are checked, uncheck the last one
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        toolbar_layout.addWidget(self.delete_duplicates_button)
        ...
        self.duplicate_list.setSelectionMode(QListWidget.MultiSelection)
    ...
    def display_results(self, pdf_files):
        ...
        for file_hash, file_paths in pdf_files.items():
            if len(file_paths) > 1:
                ...
                grouped_duplicates[original].extend(duplicates)

        # Sort the original files
        self.original_list.sortItems()

        # Add sorted and grouped duplicates to the duplicate list
        for i in range(self.original_list.count()):
            original = self.original_list.item(i).text()
            duplicates = sorted(grouped_duplicates[original])
            for duplicate in duplicates:
                self.duplicate_list.addItem(duplicate)
        ...
        if file_path in self.pdf_groups:
            for i in range(self.duplicate_list.count()):
                if self.duplicate_list.item(i).text() in self.pdf_groups[file_path]:
                    ...
                    self.duplicate_list.scrollToItem(self.duplicate_list.item(i))
            ...
            if file_path in duplicates:
                for i in range(self.original_list.count()):
                    if self.original_list.item(i).text() == original:
                        ...
                        self.original_list.scrollToItem(self.original_list.item(i))
                        self.preview_pdf(original, self.original_preview, self.original_properties)
                        break
                break

    def delete_duplicates(self):
        selected_original = self.original_list.selectedItems()
        if not selected_original:
            QMessageBox.warning(self, "No Selection", "Please select an original file to manage its duplicates.")
            return

        original_path = selected_original[0].text()
        duplicates = self.pdf_groups.get(original_path, [])

        wizard = DeleteConfirmationWizard(original_path, duplicates, self)
        if wizard.exec_():
            files_to_delete = wizard.get_selected_files()
            self.perform_deletion(original_path, files_to_delete)

    def perform_deletion(self, original_path, files_to_delete):
        deleted_count = 0
        for file_path in files_to_delete:
            try:
                ...
                if file_path == original_path:
                    items = self.original_list.findItems(file_path, Qt.MatchExactly)
                else:
                    items = self.duplicate_list.findItems(file_path, Qt.MatchExactly)
                for item in items:
                    if file_path == original_path:
                        self.original_list.takeItem(self.original_list.row(item))
                    else:
                        self.duplicate_list.takeItem(self.duplicate_list.row(item))
            except Exception as e:
                print(f"Error deleting {file_path}: {str(e)}")

        # Update the pdf_groups dictionary
        if original_path in files_to_delete:
            del self.pdf_groups[original_path]
        else:
            self.pdf_groups[original_path] = [f for f in self.pdf_groups[original_path] if f not in files_to_delete]
        ...
        self.duplicate_properties.clear()
